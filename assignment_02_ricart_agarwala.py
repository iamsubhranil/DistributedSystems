...
def generate_timestamp(ts_lock, ts_counter):
    with ts_lock:
...
class Node:
    ...
    def __init__(self, manager, id_, n, tl, tc):
    ...
    def request(self):
        ...
        self.ts.set(generate_timestamp(self.ts_lock,
                                       self.ts_counter))  # get a timestamp
        for neighbor in self.neighbors:  # ask for all tokens at once
            if neighbor == self:  # skip self
                continue
            neighbor.grant(self.semaphore, self.ts)
...
def main():
    ...
    for i in range(num_nodes):
        neighbors.append(Node(manager, i, neighbors, ts_lock, ts_counter))

    printer = Process(target=print_status, args=(neighbors,))
    printer.start()

    jobPool = Pool(processes=len(neighbors))
    jobPool.starmap(start_tick, zip(repeat(neighbors), range(len(neighbors))))
